...
For isolated Vercel Previews, generate a **Preview Deploy Key** from the Convex
project settings and set it as a Preview-only `CONVEX_DEPLOY_KEY` in Vercel.
Its value starts with `preview:`. A development deployment key does not create a
Preview backend; the build rejects that configuration before it can target the
...
| `CONVEX_DEPLOY_KEY`                            | Vercel builds           | Use a production key for Production and a project-level `preview:` key scoped only to Preview; the Preview build rejects development or production deployment keys                                             |
