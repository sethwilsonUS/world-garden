...
The installed `expo-audio` 57.0.5 native source has two background-control
...
- iOS block-based `MPRemoteCommand` registrations return opaque target tokens.
  Expo Audio 57.0.5 discards those tokens and therefore cannot remove the
...
backport for exactly `expo-audio` 57.0.5. It preflights thirteen reviewed source
